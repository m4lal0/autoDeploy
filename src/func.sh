...
function deleteApp(){
	...
	if [[ -d $PRIVESCLIN_PATH || -d $PRIVESCWIN_PATH || -d $OSINT_PATH || -d $UTILITIES_PATH || -d $WEB_PATH || -d $WIFI_PATH  || -d $WORDPRESS_PATH || -d $AD_PATH || -d $MOBILE_PATH || -d $LINUX_PATH || -d $WINDOWS_PATH || -d $PHISHING_PATH || -d $PIVOTING_PATH || -d $CLOUD_PATH || -d $IA_PATH ]]; then
		rm -rf {$PRIVESCLIN_PATH,$PRIVESCWIN_PATH,$OSINT_PATH,$UTILITIES_PATH,$WEB_PATH,$WIFI_PATH,$WORDPRESS_PATH,$AD_PATH,$MOBILE_PATH,$LINUX_PATH,$WINDOWS_PATH,$PHISHING_PATH,$PIVOTING_PATH,$CLOUD_PATH,$IA_PATH} 2>&1
